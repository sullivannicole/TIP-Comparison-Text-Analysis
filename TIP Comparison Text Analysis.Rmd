...
ctable_tot_diff <- compare_df(tot_final19, tot_draft19, c("Projnum"), keep_unchanged_cols = F)

# Input order of tables here (used in tidying up the table)
first_table <- "Final 2019"
second_table <- "Draft 2019"
...
diff_format <- ctable_tot_diff$comparison_df %>%
  ...
  gather(2:length(ctable_tot_diff$comparison_df)+1, key = "Column", value = "Value") %>%
...
# var1 <- col_comparisons$Col.x[[1]]
# var2 <- col_comparisons$Col.y[[1]]
# Var1 <- sym(var1)
# Var2 <- sym(var2)
...
# Save output as dataframe, add project number and rearrange
...
formattable(comparisons_df, list(
